...
function AsyncPirateGenerator:createScaledOutlaw(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 0.75 * scaling, 0.25 * extra, "Outlaw"%_t)
end

function AsyncPirateGenerator:createScaledBandit(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 1.0 * scaling, 0.5 * extra, "Bandit"%_t)
end

function AsyncPirateGenerator:createScaledPirate(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 1.5 * scaling, 0.65 * extra, "Pirate"%_t)
end

function AsyncPirateGenerator:createScaledMarauder(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 2.0 * scaling, 0.75 * extra, "Marauder"%_t)
end

function AsyncPirateGenerator:createScaledRaider(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 4.0 * scaling, 1.0 * extra, "Raider"%_t)
end

function AsyncPirateGenerator:createScaledBoss(position, extra)
    local extra = extra or 1.0	
    local scaling = self:getScaling()
	scaling = scaling * extra
    return self:create(position, 30.0 * scaling, 2.0 * extra, "Pirate Mothership"%_t)
end
